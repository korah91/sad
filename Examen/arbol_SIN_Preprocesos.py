...
if __name__ == '__main__':
    ...
    with open('./arbol/iris_arbol.csv', 'w') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
        filewriter.writerow(['n_iteracion', 'fScoreMacro', 'fScoreMicro', 'fScoreWeighted', 'precision', 'recall', 'max_depth', 'min_samples_leaf'])
    ...
    for valorD in range(int(d), int(D) +1, 3):
        for valorM in range(int(m), int(M) +1):
            ...
            classific_report = classification_report(testY,predictions, output_dict=True)
            ...
            iteracion = {'n_iteracion': n_iteracion,
                        # average=None porque es un problema de clasificacion binaria
                            'fScoreMacro': f1_score(testY, predictions, average='macro'),
                            'fScoreMicro': f1_score(testY, predictions, average='micro'),    
                            'fScoreWeighted': f1_score(testY, predictions, average='weighted'),  
                            'precision': precision,
                            'recall': recall,       
                            'max_depth': valorD,
                            'min_samples_leaf': valorM,
            ...
            with open('./arbol/iris_arbol.csv', 'a') as csvfile:
                filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                filewriter.writerow([iteracion['n_iteracion'], iteracion['fScoreMacro'], 
                                    ...
                                    iteracion['max_depth'], iteracion['min_samples_leaf']
                                    ])
...
import pickle
nombreModel = "modeloarbolIris.sav"
